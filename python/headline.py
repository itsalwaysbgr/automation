...
def run(playwright: Playwright) -> None:
    ...
    try:
        ...
        # Track the response status code to see if Naukri blocks the session
        response = page.goto("https://naukri.com", wait_until="domcontentloaded")
        if response:
            print(f"   -> Server responded with HTTP status code: {response.status}")
            print(f"   -> Current URL after loading: {page.url}")
        else:
            print("   -> Warning: Page navigation returned an empty response.")
        ...
        # Safe fallback logger in case the browser tab crashed completely
        try:
            print("Attempting diagnostic screenshot...")
            page.screenshot(path="error.png", full_page=True, timeout=5000)
            print("Screenshot saved to error.png")
        except Exception as screenshot_err:
            print(f"Could not save screenshot due to browser crash: {screenshot_err}")
            
        try:
            print("Attempting to dump page HTML content...")
            with open("error.html", "w", encoding="utf-8") as f:
                f.write(page.content())
            print("HTML dump saved to error.html")
        except Exception as html_err:
            print(f"Could not dump page content due to browser crash: {html_err}")
